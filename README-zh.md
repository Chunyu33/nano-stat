...
- **游戏内悬浮监控** — 透明置顶面板；8 个位置可选（四边中间 + 四角），可配置显示项、刷新间隔、仅背景透明（文字始终清晰）
- **智能 FPS 栏** — ETW 无注入采集真实帧率（反作弊安全）；按前台窗口进程统计（全屏游戏即游戏帧率），
  前台进程无帧时回退桌面整体帧率，全程可见且不被后台进程拉高
- **GPU 广泛支持** — NVIDIA 走 NVML（完整遥测）；AMD / Intel 走 WMI（基础信息）
- **CPU 温度** — 开箱即用 WMI 热区；可选安装 PawnIO 内核驱动（管理员运行）后，
  通过 LibreHardwareMonitor 桥接读取完整的 MSR 级 CPU/主板温度
